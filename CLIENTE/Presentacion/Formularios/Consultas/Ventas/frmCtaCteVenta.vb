...
Public Class frmCtaCteVenta
    ...
    Private listadoCtaCrtVta As List(Of CuentaCorrienteVenta)
    ...
    Private tareasCtaCrtVta As TareasCuentaCorienteVenta
...
#Region "LOAD"
    Private Sub FrmRemitosCompra_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        lblTitulo.Text = "Presentacion -> " & Date.Now & " -> Ingreso a CUENTAS CORRIENTES DE VENTA"

        tareasClientes = New TareasClientes()
        tareasCtaCrtVta = New TareasCuentaCorienteVenta()
...
#Region "METODOS PRIVADOS"
    Private Sub ConfigDGV()
        With dgvDetalle
            .ColumnCount = 9
            .Columns(0).Name = "idCuentaCorrienteVentas"
            ...
            .Columns("idCuentaCorrienteVentas").Visible = False
            ...
            .Columns("idCuentaCorrienteVentas").DisplayIndex = 0
    ...
    Private Async Function CargarDetalle(Optional fecDesde As Date = Nothing, Optional fecHasta As Date = Nothing) As Task
        If txtRScli.Text <> "" Then
            ...
            listadoCtaCrtVta = Await tareasCtaCrtVta.ListadoIdCliente(txtCtaCli.Text)

            If Not IsNothing(listadoCtaCrtVta) Then
                If bnfChkFactura.Checked And Not bnfChkRecibo.Checked Then
                    For i As Integer = listadoCtaCrtVta.Count - 1 To 0 Step -1
                        Dim remove As Boolean = False

                        If listadoCtaCrtVta.Item(i).Comprobante <> "FACTURA" Then
                            remove = True
                        End If

                        If fecDesde <> Nothing And fecHasta <> Nothing Then
                            If listadoCtaCrtVta.Item(i).Fecha > fecHasta Or listadoCtaCrtVta.Item(i).Fecha < fecDesde Then
                                remove = True
                            End If
                        End If

                        If remove Then
                            listadoCtaCrtVta.Remove(listadoCtaCrtVta.Item(i))
                        End If
                    Next
                ElseIf bnfChkRecibo.Checked And Not bnfChkFactura.Checked Then
                    For i As Integer = listadoCtaCrtVta.Count - 1 To 0 Step -1
                        ...
                        If listadoCtaCrtVta.Item(i).Comprobante <> "RECIBO" Then
                            remove = True
                        End If
                ...
                For Each item In listadoCtaCrtVta
                    saldoAcumulado += item.Credito - item.Debito
                    item.Saldo = saldoAcumulado

                    dgvDetalle.Rows.Add(item.IdCuentaCorrienteVentas, Format(CDate(item.Fecha), "dd/MM/yyyy"), Format(CDate(item.FechaVto), "dd/MM/yyyy"),
            ...
            ElseIf tareasCtaCrtVta.Err.Identificador <> EnumErrores.REGISTRO_NO_ENCONTRADO Then
                Notificaciones.cerrarFormProceso()
                Notificaciones.MsgBox(tareasCtaCrtVta.Err.Descripcion, Botones.OK, Iconos.ERR)
            ...
            Else
                Notificaciones.cerrarFormProceso()
                Notificaciones.MsgBox("La cuenta corriente de " + txtRScli.Text + " esta vacia.", Botones.OK, Iconos.ALERTA)
...
#Region "EVENTOS CLICK"
    ...
    Private Sub btnImprimir_Click(sender As Object, e As EventArgs) Handles btnImprimir.Click
        If dgvDetalle.Rows.Count > 0 Then
            ...
            frmReporte.rvReporte.LocalReport.ReportPath = Reportes.RPT_CUENTA_CORRIENTE_VENTA
            ...
            frmReporte.rvReporte.LocalReport.DataSources.Add(New Microsoft.Reporting.WinForms.ReportDataSource("dsCtaCteVta", New BindingSource() With {.DataSource = listadoCtaCrtVta}))
