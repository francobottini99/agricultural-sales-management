...
#Region "LOAD"
    ...
    Private Async Sub TmrLoad_Tick(sender As Object, e As EventArgs) Handles tmrLoad.Tick
        ...
        If txtOrigen.Text = "Editar" Then
            Dim detalleReserva As List(Of PedidosDetalle) = Await tareaDetRes.verDetalleReservaSinFacturarIdCliente(frmFacturaVenta.txtCtaCli.Text)

            If Not IsNothing(detalleReserva) Then
                Dim cantidad As Double = 0
                For Each item In detalleReserva
                    If item.IdPedidosDetalle = frmFacturaVenta.dgvDetalleFactura.CurrentRow.Cells("idPedidosDetalle").Value Then
                ...
                lblCantPendiente.Text = cantidad - CountCantFacturada(frmFacturaVenta.dgvDetalleFactura.CurrentRow.Cells("idPedidosDetalle").Value) + frmFacturaVenta.dgvDetalleFactura.CurrentRow.Cells("Cantidad").Value
                txtIdRes.Text = frmFacturaVenta.dgvDetalleFactura.CurrentRow.Cells("idPedido").Value
                txtIdDetallePedido.Text = frmFacturaVenta.dgvDetalleFactura.CurrentRow.Cells("idPedidosDetalle").Value
            Else
                UtilidadesTXT.ResetVariables()
                Notificaciones.cerrarFormProceso()
                Notificaciones.MsgBox(tareaDetRes.Err.Descripcion, Botones.OK, Iconos.ERR)
                Me.DialogResult = DialogResult.Cancel
                Me.Close()
            End If
        ElseIf txtOrigen.Text = "Agregar" Then
            txtCodArt.Text = frmReservaRemitoNuevoCliente.infoDetalleReserva.AscArticulos.CodArticulo
            lblCantPendiente.Text = frmReservaRemitoNuevoCliente.infoDetalleReserva.Cantidad
            txtIdRes.Text = frmReservaRemitoNuevoCliente.infoDetalleReserva.AscPedidos.IdPedidos
            txtIdDetallePedido.Text = frmReservaRemitoNuevoCliente.infoDetalleReserva.IdPedidosDetalle
        End If
    End Sub
#End Region
...
#Region "METODOS PRIVADOS"
    ...
    Private Function CountCantFacturada(ByVal idRes As Integer)
        ...
        Else
            ...
            Else
                ...
                Else
                    ...
                    For Each item As DataGridViewRow In frmFacturaVenta.dgvDetalleFactura.Rows
                        If txtIdArticulo.Text = item.Cells("idArticulo").Value And txtPrecioPPal.Text = item.Cells("Precio").Value And txtIdDetallePedido.Text = item.Cells("idPedidosDetalle").Value And item.Cells("Origen").Value = "RESERVA" Then
                            item.SetValues(txtIdArticulo.Text, txtIdRes.Text, txtIdDetallePedido.Text, 0, 0, txtCodArt.Text, txtDetalleArt.Text, lblUndSel.Text,
                                           txtPrecioPPal.Text, CDbl(item.Cells("Cantidad").Value) + CDbl(txtCantidad.Text), lblImpPPal.Text, lblAlicIvaSel.Text, impNeto, "RESERVA")
                    ...
                    If aggNuevo Then
                        frmFacturaVenta.dgvDetalleFactura.Rows.Add(txtIdArticulo.Text, txtIdRes.Text, txtIdDetallePedido.Text, 0, 0, txtCodArt.Text, txtDetalleArt.Text, lblUndSel.Text,
                                                                   txtPrecioPPal.Text, txtCantidad.Text, lblImpPPal.Text, lblAlicIvaSel.Text, impNeto, "RESERVA")
                    End If
