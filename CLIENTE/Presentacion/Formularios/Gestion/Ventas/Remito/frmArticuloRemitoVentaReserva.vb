...
Public Class frmArticuloRemitoVentaReserva
...
#Region "METODOS PRIVADOS"
    ...
    Private Function CountStkDep(ByVal idDep As Integer, ByVal idArt As Integer)
        ...
        For Each item As DataGridViewRow In frmRemitoVenta.dgvDetalleRemito.Rows
            If idDep = item.Cells("idDeposito").Value And idArt = item.Cells("idArticulo").Value Then
                If txtOrigen.Text <> "Editar" Or item IsNot frmRemitoVenta.dgvDetalleRemito.CurrentRow Then
    ...
    Private Function CountCantRet(ByVal idRes As Integer)
        Dim Count As Double = 0

        For Each item As DataGridViewRow In frmRemitoVenta.dgvDetalleRemito.Rows
            If idRes = item.Cells("idPedidosDetalle").Value Then
                Count = Count + UtilidadesVarias.SoloNumeros(item.Cells("Cantidad").Value)
            End If
        Next
    ...
    Private Sub txtCodArt_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtCodArt.KeyPress
        If Asc(e.KeyChar) = Keys.Enter Then
            ...
            Else
                ...
                frmListaArticulos.txtStock.Text = "SI"
    ...
    Private Sub txtCodDeposito_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtCodDeposito.KeyPress
        ...
        ElseIf Asc(e.KeyChar) = Keys.Enter Then
            ...
            Else
                ...
                frmListaDepositos.txtIdArt.Text = txtIdArticulo.Text
...
#Region "LOAD"
    ...
    Private Async Sub TmrLoad_Tick(sender As Object, e As EventArgs) Handles tmrLoad.Tick
        ...
        If txtOrigen.Text = "Editar" Then
            txtCodArt.Text = frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("CodArt").Value
            txtPrecioPPal.Text = frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("Precio").Value
            txtCantidad.Text = frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("Cantidad").Value
            lblSaldoReserva.Text = frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("CantidadReservada").Value - CountCantRet(frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("idPedidosDetalle").Value) + frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("Cantidad").Value
            txtIdReserva.Text = frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("idPedido").Value
            txtIdDetalleReserva.Text = frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("idPedidosDetalle").Value
        ElseIf txtOrigen.Text = "Agregar" Then
            txtCodArt.Text = frmReservaFacturaNuevoCliente.infoDetalleReserva.AscArticulos.CodArticulo
            lblSaldoReserva.Text = frmReservaFacturaNuevoCliente.infoDetalleReserva.Cantidad
            txtIdReserva.Text = frmReservaFacturaNuevoCliente.infoDetalleReserva.AscPedidos.IdPedidos
            txtIdDetalleReserva.Text = frmReservaFacturaNuevoCliente.infoDetalleReserva.IdPedidosDetalle
        End If
    End Sub
#End Region
...
#Region "EVENTOS TEXT CHANGED"
    Private Sub txtPrecioPPal_TextChanged(sender As Object, e As EventArgs) Handles txtPrecioPPal.TextChanged
        UtilidadesTXT.FormatoMonedaTXT(txtPrecioPPal, txtPrecioContable, AddressOf txtPrecioPPal_TextChanged, AddressOf txtPrecioContable_TextChanged, UtilidadesVarias.SoloNumeros(frmRemitoVenta.txtTC.Text), UtilidadesTXT.txtTipos.MONEDA_PRINCIPAL, UtilidadesTXT.txtTipos.MONEDA_CONTABLE)

        Importe()
    End Sub

    Private Sub txtPrecioContable_TextChanged(sender As Object, e As EventArgs) Handles txtPrecioContable.TextChanged
        UtilidadesTXT.FormatoMonedaTXT(txtPrecioContable, txtPrecioPPal, AddressOf txtPrecioContable_TextChanged, AddressOf txtPrecioPPal_TextChanged, UtilidadesVarias.SoloNumeros(frmRemitoVenta.txtTC.Text), UtilidadesTXT.txtTipos.MONEDA_CONTABLE, UtilidadesTXT.txtTipos.MONEDA_PRINCIPAL)
    ...
    Private Async Sub txtCodDeposito_TextChanged(sender As Object, e As EventArgs) Handles txtCodDeposito.TextChanged
        txtDeposito.Text = ""
        lblStkDep.Text = ""
        ...
        If txtCodDeposito.Text <> "" Then
            ...
            For Each item In listaDep
                If item.IdDeposito = txtCodDeposito.Text Then
                    Dim Stk As Integer = Await tareaStk.verStockFisico(txtIdArticulo.Text, item.IdDeposito)

                    If Not IsNothing(Stk) Then
                        txtDeposito.Text = item.Nombre
                        lblStkDep.Text = Stk - CountStkDep(item.IdDeposito, txtIdArticulo.Text)

                        If Not IsNothing(fijarStockFisico) Then
                            Dim key As New frmRemitoVenta.CKey With {.idArt = txtIdArticulo.Text, .idDep = item.IdDeposito}

                            If fijarStockFisico.ContainsKey(key) Then
                                lblStkDep.Text += fijarStockFisico(key)
                            End If
                        End If
                    Else
                        UtilidadesTXT.ResetVariables()
                ...
                If item.CodArticulo = txtCodArt.Text Then
                    ...
                    Else
                        ...
                        If Not IsNothing(fijarStockFisico) Then
                            ...
                            For Each key In fijarStockFisico.Keys
                                If key.idArt = item.IdArticulo Then
                                    count = fijarStockFisico(key)
                                End If
                            Next
                        ...
                        If IsNothing(StkFisicoArt) Then
                            ...
                            Me.DialogResult = DialogResult.Cancel
                            Me.Close()
                        ElseIf StkFisicoArt - CountStkArt(item.IdArticulo) = 0 Then
                            Notificaciones.cerrarFormProceso()
                            Notificaciones.MsgBox("No se dispone de Stock fisico del articulo ingresado", Botones.OK, Iconos.ERR)
                            Me.DialogResult = DialogResult.Cancel
                            Me.Close()
                        Else
                            ...
                            Else
                                ...
                                Else
                                    ...
                                    For j As Integer = listaDep.Count - 1 To 0 Step -1
                                        Dim Stk As Integer = Await tareaStk.verStockFisico(item.IdArticulo, listaDep.Item(j).IdDeposito)

                                        If Not IsNothing(Stk) Then
                                            Stk -= CountStkDep(listaDep.Item(j).IdDeposito, item.IdArticulo)

                                            If Not IsNothing(fijarStockFisico) Then
                                                Dim key As New frmRemitoVenta.CKey With {.idArt = item.IdArticulo, .idDep = listaDep.Item(j).IdDeposito}

                                                If fijarStockFisico.ContainsKey(key) Then
                                                    Stk += fijarStockFisico(key)
                                                End If
                                            End If
                                    ...
                                    If Est = "OK" Then
                                        ...
                                        lblStkFisico.Text = StkFisicoArt - CountStkArt(item.IdArticulo)
                                        ...
                                        If txtOrigen.Text = "Editar" Then
                                            txtCodDeposito.Text = frmRemitoVenta.dgvDetalleRemito.CurrentRow.Cells("idDeposito").Value
                                    ...
                                    Else
                                        UtilidadesTXT.ResetVariables()
                                        Notificaciones.cerrarFormProceso()
                                        Notificaciones.MsgBox(Est, Botones.OK, Iconos.ERR)
                                        Me.DialogResult = DialogResult.Cancel
                                        Me.Close()
                                    End If
                                End If
        ...
        Else
            ...
            Else
                If txtDeposito.Text = "" Then
                    ...
                    txtCodDeposito.Select()
                Else
                    If CInt(txtCantidad.Text) > CInt(lblStkDep.Text) Then
                        Notificaciones.MsgBox("La cantidad ingresada es mayor al stock disponible en el deposito seleccionado", Botones.OK, Iconos.ERR)

                        txtCantidad.SelectionLength = Len(txtCantidad.Text)
                        txtCantidad.Select()
                    Else
                        If CInt(txtCantidad.Text) > CInt(lblSaldoReserva.Text) Then
                            Notificaciones.MsgBox("La cantidad ingresada es mayor a la cantidad que figura en la reserva", Botones.OK, Iconos.ERR)
                        ...
                        Else
                            ...
                            If frmRemitoVenta.dgvDetalleRemito.CurrentRow IsNot Nothing And txtOrigen.Text = "Editar" Then
                                frmRemitoVenta.dgvDetalleRemito.Rows.Remove(frmRemitoVenta.dgvDetalleRemito.CurrentRow)
                            End If

                            Dim aggNuevo As Boolean = True

                            For Each item As DataGridViewRow In frmRemitoVenta.dgvDetalleRemito.Rows
                                If txtIdArticulo.Text = item.Cells("idArticulo").Value And txtPrecioPPal.Text = item.Cells("Precio").Value And txtCodDeposito.Text = item.Cells("idDeposito").Value And txtIdDetalleReserva.Text = item.Cells("idPedidosDetalle").Value And item.Cells("Origen").Value = "RESERVA" Then
                                    item.SetValues(txtIdArticulo.Text, txtCodDeposito.Text, txtIdReserva.Text, txtIdDetalleReserva.Text, 0, 0,
                                                   txtCodArt.Text, txtDetalleArt.Text, lblUndSel.Text, txtPrecioPPal.Text, CDbl(item.Cells("Cantidad").Value) + CDbl(txtCantidad.Text),
                                                   item.Cells("CantidadReservada").Value, lblImpPPal.Text, lblAlicIvaSel.Text, impNeto, txtDeposito.Text, "RESERVA")
                            ...
                            If aggNuevo Then
                                frmRemitoVenta.dgvDetalleRemito.Rows.Add(txtIdArticulo.Text, txtCodDeposito.Text, txtIdReserva.Text,
                                                                         txtIdDetalleReserva.Text, 0, 0, txtCodArt.Text, txtDetalleArt.Text, lblUndSel.Text,
                                                                         txtPrecioPPal.Text, txtCantidad.Text, lblSaldoReserva.Text, lblImpPPal.Text, lblAlicIvaSel.Text, impNeto, txtDeposito.Text, "RESERVA")
                            End If
