CREATE DEFINER=`root`@`localhost` PROCEDURE `verMovimientos_Reserva_Remito_idDetalleRemitosVenta`(
	in idDrv int(11)
)
BEGIN
	SELECT
    ...
    WHERE
		DetalleRemitosVenta.idDetalleRemitosVenta = idDrv And Movimientos_Reserva_Remito.Eliminado = "NO";
END