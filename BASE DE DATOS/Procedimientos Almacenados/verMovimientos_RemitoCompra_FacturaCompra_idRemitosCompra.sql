CREATE DEFINER=`root`@`localhost` PROCEDURE `verMovimientos_RemitoCompra_FacturaCompra_idRemitosCompra`(
	in idRC int(11)
)
BEGIN
	SELECT
    ...
    WHERE
		DetalleRemitosCompra.RemitosCompra_idRemitosCompra = idRC And Movimientos_RemitoCompra_FacturaCompra.Eliminado = "NO";
END