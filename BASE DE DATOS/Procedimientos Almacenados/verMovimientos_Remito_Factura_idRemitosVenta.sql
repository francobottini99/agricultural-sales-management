CREATE DEFINER=`root`@`localhost` PROCEDURE `verMovimientos_Remito_Factura_idRemitosVenta`(
	in idRV int(11)
)
BEGIN
	SELECT
    ...
    WHERE
		DetalleRemitosVenta.RemitosVenta_idRemitosVenta = idRV And Movimientos_Remito_Factura.Eliminado = "NO";
END