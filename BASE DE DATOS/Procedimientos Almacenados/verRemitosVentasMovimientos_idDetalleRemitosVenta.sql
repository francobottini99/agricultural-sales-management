CREATE DEFINER=`root`@`localhost` PROCEDURE `verRemitosVentasMovimientos_idDetalleRemitosVenta`(
	in idDRV int(11)
)
BEGIN
	SELECT
	...
	WHERE
		DetalleRemitosVenta.idDetalleRemitosVenta = idDRV And RemitosVentasMovimientos.Eliminado = "NO";
END